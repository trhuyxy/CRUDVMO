...
import { deleteProjectStatus } from "../project-status.services";
export const FormDetailProjectStatus = ({ projectStatusDetails, setEditStatus }) => {
  const dispatch = useDispatch();
  const { link: linkProjectStatus } = useSelector(state => state.projectStatus);
  useEffect(() => {
    dispatch(setLinkRedirect(linkProjectStatus));
  }, [linkProjectStatus]);
  const sumbitDeleteProjectStatus = () => {
    const { _id } = projectStatusDetails;
    dispatch(deleteProjectStatus(_id));
  };
  return (
